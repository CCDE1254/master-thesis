 ...
 *
 */
public class NeuralNetworkBackwardAISCall {
	int numberOfSimulations;
	int numberOfTimeSteps;
	int whichTimeStepToOptimize;//at time t_{1}, this is 1, at time t_{2}, this is 2,...
	double[][] optimizedNaturalParameters;//with size[whichTimeStepToOptimize][2]
	...
	public NeuralNetworkBackwardAISCall(int numberOfSimulations, int numberOfTimeSteps, int whichTimeStepToOptimize,
			double[][] optimizedNaturalParameters, int numberOfFirstHiddenLayerNeurons,
		...
		this.whichTimeStepToOptimize = whichTimeStepToOptimize;
		this.optimizedNaturalParameters = optimizedNaturalParameters;
	...
	public double[][] initializeWeightMatrix1() {
		double[][] weightMatrix1 = new double[numberOfSimulations*whichTimeStepToOptimize + 1][numberOfFirstHiddenLayerNeurons];
		for(int i = 0; i < (numberOfSimulations*whichTimeStepToOptimize + 1); i++) {
	...
	public double[] getInputVector() {
		double[] inputVector = new double[numberOfSimulations*whichTimeStepToOptimize + 1];
		inputVector[0] = 1.0;
		for(int i = 0; i < whichTimeStepToOptimize; i++) {
	...
	public double[] getHiddenLayer1Output(double[][] weightMatrix1) {
		...
		for(int i = 1; i < numberOfFirstHiddenLayerNeurons+1; i++) {
			double sum = 0.0;
			for(int j = 0; j < numberOfSimulations*whichTimeStepToOptimize + 1; j++) {
	...
	public double[][] getDerivativeOfWeightMatrix3(double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3, double[] output1, double[] output2, double[] output3) {
		...
		for(int j = 0; j < numberOfSimulations; j++) {
			...
			for(int i = whichTimeStepToOptimize+1; i < numberOfTimeSteps ; i++) {
				productOfDensity *= Math.pow(-2.0*optimizedNaturalParameters[i][1], 1.0/2.0)*Math.exp(Math.pow(optimizedNaturalParameters[i][0],2.0)/(4.0*optimizedNaturalParameters[i][1])
						- optimizedNaturalParameters[i][0]*normalDistributedRandomNumberMatrix[i][j] - (1.0/2.0 + optimizedNaturalParameters[i][1])*Math.pow(normalDistributedRandomNumberMatrix[i][j], 2.0));
			}
			productOfDensity *= Math.pow(-2.0*output3[1], 1.0/2.0)*Math.exp(Math.pow(output3[0],2.0)/(4.0*output3[1])
					- output3[0]*normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j] - (1.0/2.0 + output3[1])*Math.pow(normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j], 2.0));
			double value = (Math.exp(-2.0*riskFreeRate*maturity)/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][j] - strike),0.0)*payoffValid,2.0)
					*productOfDensity
					*(output3[0]/(2.0*output3[1]) - normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j]);
		...
		for(int j = 0; j < numberOfSimulations; j++) {
			...
			double productOfDensity1 = productOfDensity * Math.pow(-2.0*output3[1], 1.0/2.0)*Math.exp(Math.pow(output3[0],2.0)/(4.0*output3[1])
					- output3[0]*normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j] - (1.0/2.0 + output3[1])*Math.pow(normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j], 2.0));
			double productOfDensity2 = productOfDensity * Math.pow(-2.0*output3[1], -1.0/2.0)*Math.exp(Math.pow(output3[0],2.0)/(4.0*output3[1])
					- output3[0]*normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j] - (1.0/2.0 + output3[1])*Math.pow(normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j], 2.0));
			double value = -(Math.exp(-2.0*riskFreeRate*maturity)/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][j] - strike),0.0)*payoffValid,2.0)
					*Math.pow(-2.0*output3[1], -1.0/2.0)
					*productOfDensity1
					- (Math.exp(-2.0*riskFreeRate*maturity)/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][j] - strike),0.0)*payoffValid,2.0)
					*productOfDensity2
					*(Math.pow(output3[0],2.0)/(4.0*Math.pow(output3[1],2.0)) + normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j]*normalDistributedRandomNumberMatrix[whichTimeStepToOptimize][j]);
	...
	public double[][] getDerivativeOfWeightMatrix1(double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3, double[] output1, double[] output2, double[] output3) {
		double[][] derivativeMatrix = new double[numberOfSimulations*whichTimeStepToOptimize + 1][numberOfFirstHiddenLayerNeurons];
		...
		double[][] derivativeOutput1WeightMatrix1 = new double[numberOfSimulations*whichTimeStepToOptimize + 1][numberOfFirstHiddenLayerNeurons];
		for(int i = 0; i < numberOfSimulations*whichTimeStepToOptimize + 1; i++) {
			for(int j = 0; j < numberOfFirstHiddenLayerNeurons; j++) {
