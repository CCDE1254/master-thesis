 ...
 *
 */
public class NeuralNetworkISCall {
	...
	public NeuralNetworkISCall(int numberOfSimulations, int numberOfTimeSteps, int numberOfFirstHiddenLayerNeurons,
	...
	public double[][] initializeWeightMatrix1() {
		double[][] weightMatrix1 = new double[numberOfTimeSteps + 1][numberOfFirstHiddenLayerNeurons];
		for(int i = 0; i < (numberOfTimeSteps + 1); i++) {
			for(int j = 0; j < numberOfFirstHiddenLayerNeurons; j++) {
				weightMatrix1[i][j] = 0.0001;
			}
		}
	...
	public double[] getHiddenLayer2Output(double[] inputVector, double[][] weightMatrix1, double[][] weightMatrix2) {
		...
		for(int i = 1; i < numberOfSecondHiddenLayerNeurons+1; i++) {
			...
			for(int j = 0; j < numberOfFirstHiddenLayerNeurons + 1; j++) {
				sum += weightMatrix2[j][i-1]*inputs[j];
			}
	...
	public double[] getOutputLayerOutput(double[] inputVector, double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3) {
		...
		for(int i = 0; i < 2; i++) {
			...
			for(int j = 0; j < numberOfSecondHiddenLayerNeurons + 1; j++) {
				sum += weightMatrix3[j][i]*inputs[j];
			}
			outputs[i] = sum;
		}
		return outputs;
	...
	public double[] getOutput(double[] inputVector, double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3) {
		...
		outputs3[0] = inputs[0];
		outputs3[1] = Math.min(inputs[1],0);
		return outputs3;
	}
	...
	public double[][] getDerivativeOfWeightMatrix3(int iterationStep, double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3, double[] output1, double[] output2, double[] output3) {
		...
		for(int i = 0; i < timeSeries.length; i++) {
			...
			sumOfNormalNumbers += normalDistributedRandomNumberMatrix[i][iterationStep];
			sumOfSquaredNormalNumbers += Math.pow(normalDistributedRandomNumberMatrix[i][iterationStep], 2.0);
		}
		double derivativeLossFunctionEta1 = (Math.exp(-2.0*riskFreeRate*maturity)/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][iterationStep] - strike),0.0)*payoffValid,2.0)
		...
		double derivativeLossFunctionEta2 = -(Math.exp(-2.0*riskFreeRate*maturity)*numberOfTimeSteps/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][iterationStep] - strike),0.0)*payoffValid,2.0)
					...
					*(Math.exp(numberOfTimeSteps*Math.pow(output3[0], 2.0)/(4.0*output3[1]) - output3[0]*sumOfNormalNumbers - (1.0/2+output3[1])*sumOfSquaredNormalNumbers))
					+ (Math.exp(-2.0*riskFreeRate*maturity)/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][iterationStep] - strike),0.0)*payoffValid,2.0)
					*Math.pow(-2.0*output3[1], numberOfTimeSteps/2.0)
	...
	public double[][] getDerivativeOfWeightMatrix1(int iterationStep, double[] inputVector,  double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3, double[] output1, double[] output2, double[] output3) {
		...
		double[][] derivativeOutput1WeightMatrix1 = new double[numberOfTimeSteps + 1][numberOfFirstHiddenLayerNeurons];
		for(int i = 0; i < numberOfTimeSteps + 1; i++) {
			for(int j = 0; j < numberOfFirstHiddenLayerNeurons; j++) {
				if(getHiddenLayer1OutputBeforeReLU(inputVector, weightMatrix1)[j+1]>0) {
					derivativeOutput1WeightMatrix1[i][j] = inputVector[i];
				}else {
					derivativeOutput1WeightMatrix1[i][j] = -inputVector[i];
				}
