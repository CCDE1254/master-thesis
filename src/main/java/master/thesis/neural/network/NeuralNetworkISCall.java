...
public class NeuralNetworkISCall {
	...
	public double[][] getDerivativeOfWeightMatrix3(double[][] weightMatrix1, double[][] weightMatrix2, double[][] weightMatrix3, double[] output1, double[] output2, double[] output3) {
		...
		for(int j = 0; j < numberOfSimulations; j++) {
			...
			double value = -(Math.exp(-2.0*riskFreeRate*maturity)*numberOfTimeSteps/numberOfSimulations)*Math.pow(Math.max((underlyingPriceMatrix[timeSeries.length-1][j] - strike),0.0)*payoffValid,2.0)
