 ...
 *
 */
public class DoubleBarrierKnockOutCall {
	...
	public DoubleBarrierKnockOutCall(double initialStockPrice, double riskFreeRate, double volatilityTerm,
	...
	public double getAnalyticValue() {
		...
		for(double n = -10.0; n <= 10.0; n++) {//here let n loop from -10 to 10
			...
			double F = upperBoundFactorB*Math.exp(upperBoundExponentialDelta1*maturity);
			double tau = maturity;
			double d1n = ( Math.log(initialStockPrice*Math.pow(upperBoundFactorB, 2.0*n)/(strike*Math.pow(lowerBoundFactorA, 2.0*n))) + (riskFreeRate + Math.pow(volatilityTerm, 2.0)/2.0)*tau )
					/(volatilityTerm*Math.sqrt(tau));
			double d2n = ( Math.log(initialStockPrice*Math.pow(upperBoundFactorB, 2.0*n)/(F*Math.pow(lowerBoundFactorA, 2.0*n))) + (riskFreeRate + Math.pow(volatilityTerm, 2.0)/2.0)*tau )
					/(volatilityTerm*Math.sqrt(tau));
			double d3n = ( Math.log(Math.pow(lowerBoundFactorA, 2.0*n+2.0)/(strike*initialStockPrice*Math.pow(upperBoundFactorB, 2.0*n))) + (riskFreeRate + Math.pow(volatilityTerm, 2.0)/2.0)*tau )
					/(volatilityTerm*Math.sqrt(tau));
			double d4n = ( Math.log(Math.pow(lowerBoundFactorA, 2.0*n+2.0)/(F*initialStockPrice*Math.pow(upperBoundFactorB, 2.0*n))) + (riskFreeRate + Math.pow(volatilityTerm, 2.0)/2.0)*tau )
					/(volatilityTerm*Math.sqrt(tau));
		...
		double callPrice = initialStockPrice * sumPart1 - strike * Math.exp(-riskFreeRate*maturity) * sumPart2;
		return callPrice;
	}
